...
class Eye_Dataset(Dataset):
    def __init__(self, path,transform=None):
        ...
        self.Y = [torch.tensor([float(k) for k in x.split(',')[1:]]) for x in self.file.readlines()]
        
        self.file.close()
        
        self.file = open(path)
        self.imgpath = [x.split(',')[0] for x in self.file.readlines()[2:]]
        self.file.close()
...
def trainer(model,loss_fn=lossfn,epoch=5,rate=1e-3):
    ...
    for i in tqdm.tnrange(epoch, desc = 'epoch'):
        print('ding')
